...
def count_selectable_days(page: Page) -> int:
    ...
    """
    count = 0
    for i in range(CALENDAR_RETRY_ATTEMPTS):
        try:
        ...
        if count > 0:
            ...
            return count
        if i < CALENDAR_RETRY_ATTEMPTS - 1:
            time.sleep(CALENDAR_RETRY_INTERVAL_SEC)
    return count
...
def read_seat_panel(page: Page) -> str:
    """
    日付クリック後の残席パネルを読む。

    このパネルは非同期で後から更新されるため、更新前に読むと空文字や
    古い内容が返り、残席0と誤判定してしまう(=空きがあるのに通知されない)。
    「残り○名」の表記が現れるまで、短い間隔で読み直す。
    """
    text = ""
    for i in range(SEAT_PANEL_ATTEMPTS):
        try:
            text = page.locator("#visitTimeList").inner_text()
        except Exception:
            text = ""
        if re.search(config.SEAT_TEXT_REGEX, text or ""):
            if i > 0:
                log(f"  (残席パネルの反映を {i} 回待ち直して読み取り成功)")
            return text
        if i < SEAT_PANEL_ATTEMPTS - 1:
            time.sleep(SEAT_PANEL_INTERVAL_SEC)
    return text
...
def append_history(venue: str, changes: list) -> None:
    """
    変化があったものだけを history_<試験場>.jsonl に追記する。
    ...
    try:
        ...
        log(f"[{venue}] 変化 {len(changes)} 件を {path} に記録しました")
    except OSError as e:
...
def detect_changes(state: dict, venue: str, venue_results: dict, month_keys: list) -> list:
    """
    state を更新する前に呼び出し、前回からの変化を洗い出す。
    戻り値: [(slot_key, 変化前, 変化後), ...]

    「新規」= その日付が新しくカレンダー上で選択可能になった、の意味。
    残席0のまま出現したのか、席付きで出現したのかを区別するために記録する。
    """
    prev_seats = {k: v.get("seats", 0) for k, v in state["slots"].items()}
    prev_known = set(state["slots"].keys())
    changes = []
    observed = set()
    ...
    for date_str, seats in sorted(venue_results.items()):
        for ampm in ("am", "pm"):
            ...
            observed.add(key)
            after = seats[ampm]
            if key not in prev_known:
                changes.append((key, "新規", after))
            elif prev_seats.get(key, 0) != after:
                changes.append((key, prev_seats.get(key, 0), after))
...
def reset_unseen_slots(state: dict, venue: str, venue_results: dict, month_keys: list) -> int:
    ...
    notified が true のまま永久に残り、その枠が再び空いても通知されなくなる。
    """
    observed = set()
